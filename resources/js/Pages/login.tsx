...
export function PosLoginForm({
  ...
  const handleLogin = async (event?: React.SyntheticEvent) => {
    ...
    try {
      ...
      const redirect =
        response.data?.redirect || (role === "ADMIN" ? "/admin" : "/kasir");
    ...
    <div className={`mx-auto w-full max-w-sm ${className}`}>
      ...
      {loginMethod === "CREDENTIALS" ? (
        <form
          ...
          className="space-y-3.5 animate-in fade-in slide-in-from-right-4 duration-300"
        >
          ...
          <div className="flex items-center justify-between gap-4 px-1 text-sm">
            ...
            <Link
              href="/lupa-password"
              className="font-semibold text-[#6d4c36] hover:text-[#2f241c]"
            >
              Lupa password?
            </Link>
          </div>
        ...
        <div className="flex flex-col items-center animate-in fade-in slide-in-from-left-4 duration-300">
          <div className="mb-2 text-center">
            <p className="text-sm font-semibold text-[#2f241c]">
              Mode PIN untuk kasir
            </p>
            <p className="mt-1 text-xs leading-5 text-[#806049]">
              Gunakan PIN perangkat yang sudah terdaftar untuk masuk lebih
              cepat.
            </p>
          </div>
          ...
          <Link
            href="/lupa-pin"
            className="mt-5 text-sm font-semibold text-[#6d4c36] hover:text-[#2f241c]"
          >
            Lupa PIN?
          </Link>
        </div>
                ...
                <div className="relative z-10 space-y-4 rounded-[1.5rem] border border-white/70 bg-white/70 p-5 shadow-[0_22px_48px_-36px_rgba(58,33,23,0.28)]">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm leading-6 text-[#806049]">
                      Supervisor dan kasir memakai alur yang sama, dengan
                      validasi jelas dan akses yang tetap aman.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="flex w-full flex-1 flex-col justify-between overflow-y-auto bg-[linear-gradient(180deg,rgba(255,252,247,0.84),rgba(255,248,238,0.98))] p-6 sm:p-8 lg:p-8">
          ...
          <div className="mt-5 flex items-center justify-between gap-4 text-xs text-[#8d6b4e]">
            <Link
              href="/"
              prefetch
              className="inline-flex items-center gap-2 rounded-xl border border-[#dfcfbb] bg-[#fff8ef] px-3 py-2 font-semibold text-[#6d4c36] shadow-sm transition-colors hover:bg-[#f7eddc] hover:text-[#2f241c] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-[#8e6847]"
            >
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <div className="min-w-0">
        <h1
          className={`font-display font-semibold tracking-[-0.04em] text-[#3a2117] ${compact ? "text-2xl" : "text-4xl"}`}
        >
          Cahaya Rasa
        </h1>
      </div>
