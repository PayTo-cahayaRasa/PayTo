    ...
    return <>
        ...
        <main className="min-h-screen bg-[#fffaf3] px-4 py-10 text-[#3a2117] sm:px-6">
            <div className="mx-auto max-w-3xl">
                ...
                <p className="mt-3 text-[#806049]">Simpan nomor pesanan ini dan selesaikan pembayaran sesuai instruksi.</p>

                <section className="mt-8 border-y border-[#dfcfbb] py-7">
                    ...
                    {isBankTransfer && <div className="mt-6 space-y-1"><p className="text-sm text-[#806049]">Transfer bank</p><p className="text-xl font-bold">{payment.bank_name || 'Rekening belum dikonfigurasi'}</p><p>{payment.bank_account_number} {payment.bank_account_name && `a.n. ${payment.bank_account_name}`}</p></div>}
                    {isQris && <div className="mt-6"><p className="mb-3 font-bold">Pindai QRIS</p>{payment.qris_image_url ? <img src={payment.qris_image_url} alt="Kode QRIS pembayaran toko" className="max-h-80 max-w-full border border-[#dfcfbb] bg-white p-3" /> : <p className="text-[#806049]">Gambar QRIS belum dikonfigurasi. Hubungi toko melalui WhatsApp.</p>}</div>}
                    {order.payment_method === 'PAY_AT_STORE' && <p className="mt-6 font-semibold">Bayar di toko saat mengambil pesanan.</p>}
                    <p className="mt-5 text-sm text-[#806049]">{payment.instructions}</p>
                    {order.payment_method !== 'PAY_AT_STORE' && <button type="button" onClick={copyPayment} className="mt-5 min-h-11 rounded-full border border-[#3a2117] px-5 text-sm font-bold">{copied ? 'Informasi tersalin' : 'Salin informasi pembayaran'}</button>}
                </section>
                ...
                <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                    ...
                    <a href={trackingUrl} className="inline-flex min-h-12 items-center justify-center rounded-full bg-[#3a2117] px-6 text-sm font-bold text-white">Lacak Pesanan</a>
