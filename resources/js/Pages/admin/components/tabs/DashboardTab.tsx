    ...
    return (
        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 gap-4 sm:gap-6 lg:grid-cols-3">
                <section className="min-h-[280px] rounded-2xl sm:rounded-[2rem] border border-[var(--color-cream-200)] bg-[linear-gradient(180deg,rgba(255,252,247,0.96),rgba(249,243,234,0.92))] p-4 sm:p-6 shadow-[0_24px_44px_-36px_rgba(58,33,23,0.28)] lg:col-span-2">
                    <div className="mb-4 sm:mb-6 flex items-center justify-between">
                        <div>
                            <p className="text-[10px] sm:text-xs font-semibold uppercase tracking-[0.18em] text-[#8e6847]">Ringkasan mingguan</p>
                            <h3 className="mt-1 text-lg sm:text-xl font-semibold tracking-[-0.03em] text-[var(--color-cocoa-900)]">Trend Penjualan (7 Hari)</h3>
                        </div>
                        <span className="rounded-full bg-[#edf5ee] px-2.5 py-1 text-[11px] sm:text-xs font-semibold text-[var(--color-leaf-600)]">Real-time</span>
                    </div>

                    <div className="flex h-44 sm:h-48 items-end justify-between gap-1.5 sm:gap-4 px-1 sm:px-4">
                        {isLoading ? (
                            ...
                            weeklyTrend.map((point) => {
                                ...
                                return (
                                    <div key={point.date} className="group relative w-full rounded-t-xl bg-[#f1e6d7]">
                                        ...
                                        ></div>
                                        <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[9px] sm:text-[10px] font-semibold text-[#8d6b4e] opacity-0 transition group-hover:opacity-100 whitespace-nowrap">
                            ...
                            <div className="w-full rounded-[1.5rem] border border-dashed border-[var(--color-cream-300)] bg-[#fffdf9] px-4 py-5 text-sm text-[var(--color-cocoa-500)] text-center">
                                Belum ada data penjualan minggu ini.
                            </div>
                        )}
                    </div>

                    <div className="mt-4 flex justify-between px-1 text-[11px] sm:text-xs font-medium text-[#9b7a5c]">
                        {weeklyTrend.length
                            ? weeklyTrend.map((point) => {
                ...
                <section className="rounded-2xl sm:rounded-[2rem] border border-[var(--color-cream-200)] bg-[linear-gradient(180deg,rgba(255,252,247,0.96),rgba(249,243,234,0.92))] p-4 sm:p-6 shadow-[0_24px_44px_-36px_rgba(58,33,23,0.28)] lg:col-span-1">
                    <h3 className="mb-4 text-lg sm:text-xl font-semibold tracking-[-0.03em] text-[var(--color-cocoa-900)]">Aktivitas Terkini</h3>
                            ...
                            dashboard.recent_activities.map((activity) => (
                                <div key={activity.id} className="flex items-start gap-3">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <p className="text-sm font-semibold text-[var(--color-cocoa-900)] truncate">{activity.title}</p>
                                        <p className="text-xs text-[var(--color-cocoa-500)] truncate">
            ...
            <section className="rounded-2xl sm:rounded-[2rem] border border-[var(--color-cream-200)] bg-[linear-gradient(180deg,rgba(255,252,247,0.96),rgba(249,243,234,0.92))] p-4 sm:p-6 shadow-[0_24px_44px_-36px_rgba(58,33,23,0.28)]">
