    ...
    return (
        <div className="flex-1 overflow-y-auto pr-2 pb-4 -mr-2 custom-scrollbar-light animate-in slide-in-from-bottom-4 duration-300">
            <div className="space-y-6">
                <section>
                    <h3 className="text-sm font-bold text-cocoa-400 uppercase tracking-wider mb-3 ml-2">Perangkat Keras</h3>
                    <div className="bg-white/40 backdrop-blur-md border border-white/60 rounded-4xl overflow-hidden shadow-sm">
                        <div className="p-4 border-b border-white/40 flex items-center justify-between hover:bg-white/30 transition-colors">
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 rounded-xl bg-snack-100 text-snack-600 flex items-center justify-center">
                                    <Printer size={20} />
                                </div>
                                <div>
                                    <div className="font-bold text-cocoa-800">Printer Thermal</div>
                                    <div className="text-xs text-leaf-600 font-medium flex items-center gap-1">
                                        <span className={`w-1.5 h-1.5 rounded-full ${printerStatus === 'connected' ? 'bg-leaf-500' : 'bg-cocoa-300'}`}></span>
                            ...
                            <button
                                ...
                                className="px-4 py-2 bg-white text-cocoa-600 text-xs font-bold rounded-lg border border-cocoa-200 hover:bg-cocoa-50 disabled:opacity-50"
                            >
                                {isTesting ? 'Testing...' : 'Test Print'}
                            </button>
                        </div>
                        <div className="p-4 flex flex-col gap-3">
                            <div className="text-xs font-bold text-cocoa-500 uppercase tracking-wider">Hubungkan Printer</div>
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                                <input
                                    ...
                                    className="flex-1 rounded-xl border border-cocoa-200 bg-white px-3 py-2 text-sm font-medium text-cocoa-700"
                                />
                                <button
                                    onClick={handleSavePrinter}
                                    disabled={isSaving || !printerName.trim()}
                                    className="px-4 py-2 bg-snack-600 text-white text-xs font-bold rounded-lg shadow-lg shadow-snack-200 hover:bg-snack-700 disabled:opacity-50"
                                >
                ...
                <div className="text-center pt-4 pb-8">
                    <div className="text-xs font-bold text-cocoa-400">PayTo v1.0.1</div>
                </div>
            </div>
        </div>
    );
